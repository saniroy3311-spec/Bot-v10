...
def run_backtest(df: pd.DataFrame, signal_log_path: Optional[str] = None) -> list[BTTrade]:
    ...
    for i in range(1, n):
        ...
        if in_position and cur is not None:
            # Skip trail/BE logic on entry bar (i == entry_bar_idx) - Pine evaluates
            # at bar close, so trail would only activate from NEXT bar after entry.
            if i > entry_bar_idx:
                if cur_is_long:
                ...
                # On entry bar, just update peak_price from entry bar high/low
                if cur_is_long:
                    peak_price = max(peak_price, high)
                else:
                    peak_price = min(peak_price, low)
